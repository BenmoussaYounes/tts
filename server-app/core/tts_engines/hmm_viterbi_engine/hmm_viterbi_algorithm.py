import re
import pickle #This remembers the object that has been taught. Very important because otherwise it would have to relearn.
import re #Helps with word manipulations. In this context, for easy string subbing.
from core.tts_engines.hmm_viterbi_engine.text_analysis.word_spliter import wordSplitter
from pydub import AudioSegment #Important for putting the Wavs together.
from playsound import playsound #Needed to play the created mashed together sounds
from core.tts_engines.hmm_viterbi_engine.utils.constants import path
#Tempfile is really only here to create a random filename
import tempfile

wavFile = f'{path}phonemes'
exceptions = {
    'I':'AY',
    'YOU':'Y UW',
    'WE':'W IY',
...
class Pronounce:
    def __init__(self):
        ...
        self.states = () #The phonemes. Aka the hidden states.
        #Emission probabilities of the hidden states.
        self.emit_p = {} #How likely it is to output an observable state.
        #Transition probabilities between the hidden states.
        self.trans_p = { }#How likely it is to transition to another phoneme at next state.
        self.initial = [{}, 0] #The likelihood of starting with a certain phoneme.
        self.emits = () #This is for debugging, but it tells all the observable states.

    def pronounce(self, inp, debug=False):
        #inp = inp.upper() #Gets the input and puts it in uppercase format, which my HMM learned in.
        #Splits the sentence
        sent = inp.upper()
        sent = re.split('\.|,', sent)
        ...
        sentence.export(f'{path}generated_speech/{inp}{fName}.wav', format='wav')
        #playsound(f'{wavFile}/1{fName}.wav') this is causing an error
        ...
        return sentPro, f'{path}generated_speech/{inp}{fName}.wav'
...
def createObj():
    train_dataset_path = f"{path}useableWords.txt"
    print("Training the model... {train_dataset_path}")
    hmm = Pronounce()
    hmm.addStates(train_dataset_path)
    hmm.learn(train_dataset_path)
    pickle.dump(hmm, open(f'{path}hmm_trained_model.p', 'wb'))
    return hmm

def getObj():
    hmm = pickle.load(open(f"{path}hmm_trained_model.p", "rb"))
    return hmm